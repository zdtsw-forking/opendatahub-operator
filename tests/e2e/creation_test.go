...
func (tc *testContext) testAllComponentCreation(t *testing.T) error { //nolint:funlen,thelper
	...
	dscErr := wait.PollUntilContextTimeout(tc.ctx, 2*generalRetryInterval, componentReadyTimeout, false, func(ctx context.Context) (bool, error) {
		if tc.testDsc.Status.Phase != "Ready" {
			fmt.Println("error getting DSC to Ready status :%v" + tc.testDsc.Status.Phase)
			return false, nil
		}
		return true, nil
	})
