...
// +kubebuilder:resource:scope=Cluster,shortName=dsccfo
...
type DSCCodeFlare struct {
	...
	Spec   DSCCodeFlareSpec   `json:"spec,omitempty"`
	Status DSCCodeFlareStatus `json:"status,omitempty"`
}

type DSCCodeFlareSpec struct {
	DSCComponentSpec `json:",inline"` // Embedded DSCComponentSpec
}

// CDSCCodeFlareStatus defines the custom status of DSCCodeFlare.
