...
// +kubebuilder:resource:scope=Cluster,shortName=dscwb
// +kubebuilder:storageversion
// +kubebuilder:printcolumn:name="Status",type=string,JSONPath=.status.phase,description="Status"
// +kubebuilder:printcolumn:name="Age",type=date,JSONPath=.metadata.creationTimestamp,description="The age of the resource"
type DSCWorkbench struct {
	metav1.TypeMeta   `json:",inline"`
	metav1.ObjectMeta `json:"metadata,omitempty"`

	Spec   WBComponentSpec   `json:"spec,omitempty"`
	Status WBComponentStatus `json:"status,omitempty"`
}
...
// WBComponentStatus defines the custom status of DSCCodeFlare.
