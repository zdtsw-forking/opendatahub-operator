	...
	if enabled {
		if (dscispec.DevFlags == nil || dscispec.DevFlags.ManifestsUri == "") && (w.DevFlags == nil || len(w.DevFlags.Manifests) == 0) {
			if platform == deploy.ManagedRhods || platform == deploy.SelfManagedRhods {
				...
				if err := deploy.ApplyParamsNew(notebookControllerPath, w.SetImageParamsMap(imageParamMap), false); err != nil {
					return err
				}
				// for odh-notebook-controller image
				if err := deploy.ApplyParamsNew(kfnotebookControllerPath, w.SetImageParamsMap(imageParamMap), false); err != nil {
					return err
				}
			}
		}
