...
func (m *ModelMeshServing) ReconcileComponent(ctx context.Context,
	...
	if enabled {
		...
		if (dscispec.DevFlags == nil || dscispec.DevFlags.ManifestsUri == "") && (m.DevFlags == nil || len(m.DevFlags.Manifests) == 0) {
			if err := deploy.ApplyParamsNew(Path, m.SetImageParamsMap(imageParamMap), false); err != nil {
				return err
			}
		}
	}
	...
	if enabled {
		...
		if dscispec.DevFlags == nil || dscispec.DevFlags.ManifestsUri == "" {
			if err := deploy.ApplyParamsNew(DependentPath, m.SetImageParamsMap(dependentImageParamMap), false); err != nil {
				return err
			}
		}
	}
